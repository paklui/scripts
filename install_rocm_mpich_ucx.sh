...
export MPICH_DIR=$INSTALL_DIR/mpich
...
# sudo yum install -y automake autoconf libtool m4 numactl-devel libpciaccess-devel binutils-devel pdflatex doxygen libpciaccess-devel
...
# Function to setup MPICH
SetupMPICH () {
    if [ "$1" = true ]; then
        echo "Cloning fresh copy of MPICH"
        rm -rf mpich
        #wget https://www.mpich.org/static/downloads/4.0.1/mpich-4.0.1.tar.gz
        git clone git@github.com:pmodels/mpich.git -b 4.0.x || return 1
        cd mpich || return 1
    ...
    cd build || return 1
    ../configure --prefix=$MY_MPICH_DIR --with-device=ch4:ucx --with-ucx=${UCX_DIR} --enable-fast=O3 --enable-g=dbg --with-hip=${ROCM_DIR} || return 1
    #../configure --prefix=$MY_MPICH_DIR --with-device=ch4:ucx --enable-fast=O3 --enable-g=dbg --with-hip=${ROCM_DIR} || return 1
    make -j 8 || return 1
...
SetupMPICH         $1 || { echo "[ERROR] Unable to install MPICH";  exit 1; }
